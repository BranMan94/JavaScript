...
let isPrime = false; // Assume the number is not prime initially

while (!isPrime) {
  let number = parseInt(prompt("Enter a whole number:")); // Prompt the user for input and convert it to an integer

  isPrime = true; // Reset the flag to true for each new input
  ...
  if (isPrime) {
    ...
    console.log(number + " is not a prime number. Please try again.");
  }
}
